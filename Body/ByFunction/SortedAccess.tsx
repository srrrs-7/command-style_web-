...
import {
    GetAllCodesSortedAccessQueryVariables,
    useGetAllCodesSortedAccessQuery,
...
function SortedAccess() {
    ...
    const variable: GetAllCodesSortedAccessQueryVariables = { limit: 30, skip: 30 * (page - 1) };
    const { data, isError, refetch, failureCount } = useGetAllCodesSortedAccessQuery(client, variable, option, NewHeader());
    if (failureCount == 2) {
        // RemoveCookie();
    ...
    return (
        <>
            ...
            <div>
                {data?.GetAllCodesSortedAccess.length == 0 ? (
                    <div className={styles.flex_box}>
                        <p>No items</p>
                    </div>
                ) : (
                    <div className={styles.flex_box}>
                        <p>{data?.GetAllCodesSortedAccess.length} items</p>
                    </div>
                )}
            </div>
            ...
            <div className={styles.box}>
                <div className={styles.grid_box}>
                    {data?.GetAllCodesSortedAccess.map((c) => (
